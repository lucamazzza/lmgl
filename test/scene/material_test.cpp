...
TEST_F(MaterialTest, NamedConstructor) {
    EXPECT_EQ(material->get_name(), "TestMaterial");
}
...
TEST_F(MaterialTest, DefaultAlbedo) {
    EXPECT_EQ(material->get_albedo(), glm::vec3(1.0f, 1.0f, 1.0f));
}
...
TEST_F(MaterialTest, DefaultRoughness) {
    EXPECT_FLOAT_EQ(material->get_roughness(), 0.5f);
}
...
TEST_F(MaterialTest, DefaultAO) {
    EXPECT_FLOAT_EQ(material->get_ao(), 1.0f);
}
...
TEST_F(MaterialTest, DefaultEmissive) {
    EXPECT_EQ(material->get_emissive(), glm::vec3(0.0f, 0.0f, 0.0f));
}
