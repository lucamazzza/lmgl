...
std::shared_ptr<scene::Node> ModelLoader::load(const std::string& fpath, 
...
std::shared_ptr<scene::Node> ModelLoader::process_node(aiNode* ai_node,
    ...
    for (unsigned int i = 0; i < ai_node->mNumMeshes; ++i) {
        ...
        if (mesh) {
            if (ai_node->mNumMeshes == 1) {
                ...
                auto mesh_node = std::make_shared<scene::Node>(
                        std::string(ai_mesh->mName.C_Str()) + "_mesh_" + std::to_string(i)
                );
...
std::shared_ptr<scene::Mesh> ModelLoader::process_mesh(aiMesh* ai_mesh,
                                                       const aiScene* ai_scene,
                                                       const std::string& dir,
                                                       std::shared_ptr<renderer::Shader> shader) {
